...
const generateAccessAndRefreshToken = async (userId) => {
    try {
        ...
        await user.save({ validetaBeforeSave: false })
...
const registerUser = asyncHandler( async(req, res) => {
    // res.status(200).json({
    //     message : "Every thing is possible in this fake world! you can conquere this world"
    // }) 
   ...
   const existedUser = await User.findOne({
    ...
    if (existedUser) {
     ...
     // const coverImageLcoalPath = req.files?.coverImage[0]?.path;

     let coverImageLcoalPath;
     if (req.files && Array.isArray(req.files.coverImage) && req.files.coverImage.length>0) {
        coverImageLcoalPath = req.files.coverImage[0].path
    ...
    const coverImage = await uploadOnCloudinary(coverImageLcoalPath);
...
const loginUser = asyncHandler( async (req, res) => {
    ...
    // (!(username || email)) both have same working
    if (!username && !email) {
    ...
    if (!isPasswordValide) {
        throw new ApiError(401, "Password invalide")
